    ...
    request.onload = function() {
      if (request.status >= 200 && request.status < 400) {
        ...
        for (var group in groups)
        {
            ...
            outputHtml += '<p>'  +Object.values(groups[group]) + " " + Object.keys(groups[group]) ;
            outputHtml += `<button id=${Object.keys(groups[group])} onclick='deleteRouteGroup(this.id)'>Delete </button>`;
            outputHtml += '</p>';
            outputHtml += '</div>\n';
        }

        outputHtml += '<button class="pure-button pure-button-primary">+ Add More</button>';

        groupList.innerHTML = outputHtml;

        // TODO: This needs to be an awaitable in the future
        document.querySelector('#route-collection > .pure-button').addEventListener('click', moveToObjectCreation, false);
...
async function deleteRouteGroup(objectId)
{
  await fetch(`/deleteRouteGroup`, {
    method: 'POST',
    headers: {"Content-Type":"application/json"},
    body: JSON.stringify({userId: localStorage['userId'], objectId: objectId}),
  })
  .catch((err) => {
...
async function getRouteGroup(data)
{
  if (data.constructor != Object) {
    console.log('Error: data is not a dictionary!');
    return false;
  }
  ...
  let response = await fetch(`/getRouteGroup`, {
    method: 'POST',
    headers: {"Content-Type":"application/json"},
    body: JSON.stringify({userId: localStorage['userId'], filters: data}),
  })
  .catch((err) => {
    console.log('Error:', err)
    return false;
...
*/
async function updateRouteGroup(objectId, updatedRouteGroup)
{
...
function addGroupRoute()
{
    ...
    var data =
    {
        ...
        "objectName"        :   variableHandling(document.querySelector('#object-name').value, "class"),
        ...
        "isReadMultiple"    :   document.querySelector('#read-multiple-bool').innerText == "Yes",
        "isReadAllObjects"  :   document.querySelector('#read-objects-bool').innerText == "Yes",
        "isSingleRoute"     :   document.querySelector('#single-route-bool').innerText == "Yes",
    }
...
async function deleteObjectAttribute(attributeId)
{
...
*/
async function updateObjectAttribute(attributeId, updatedObjectAttribute)
{
  let resStream = await fetch(`/updateObjectAttribute`, {
    method: 'POST',
    headers: {"Content-Type":"application/json"},
    body: JSON.stringify({updatedObjectAttribute, attributeId, userId: localStorage['userId']}),
  })
  .catch((err) => {
    console.log('Error:', err)
    return false;
...
function flipBoolButton(event)
{
    ...
    {
        event.srcElement.className = "bool-button pure-u-1-3 pure-button pure-button-active"
...
let attrNumber = 0;
function addAdditionalAttributeSpace()
{
    attrNumber++;
...
`;
    ...
    document.querySelector(`#dropdown${attrNumber}`).setAttribute("onfocus", "isDropdownFocused=true")
    document.querySelector(`#dropdown${attrNumber}`).setAttribute("onblur", "isDropdownFocused=false")
}
...
function generateAPI()
{
    ...
    for (var row of attributeRows)
    {
        ...
        attributeList.push(
            {
                'name'              :   variableHandling(row.querySelector('td > .attribute-name').value, "var"),
...
function pullGeneratedRoutes(objectId)
{
    var request = new XMLHttpRequest();
...
function moveToObjectAttributeCreation()
{
    page = "attrCreation"
    console.log("moving to object attribute creation");
    document.querySelector('#create-object-name-span').setAttribute("hidden", "");
    document.querySelector('#define-object-attributes-span').removeAttribute("hidden");
    addAdditionalAttributeSpace()
}

function moveToObjectAttributeCreationFromGenRoutes()
{
    console.log("moving back to object attribute creation after generated routes");
    document.querySelector('#object-routes-view-span').setAttribute("hidden", "");
    document.querySelector('#define-object-attributes-span').removeAttribute("hidden");
}

function moveToRouteDisplay(objectId)
{
    page = "routeDisplay"
    pullGeneratedRoutes(objectId);
    console.log("moving to object route display / post api generation");
    document.querySelector('#define-object-attributes-span').setAttribute("hidden", "");
    document.querySelector('#object-routes-view-span').removeAttribute("hidden");
}

async function grabFakeUser()
{
...
function generateFullAPI(serverType)
{
    var request = new XMLHttpRequest();
    ...
    request.onload = function() {
      if (request.status >= 200 && request.status < 400) {
        ...
        const trimmedURL = currentURL[currentURL.length -1] === '?' ? currentURL.substr(0, currentURL.length - 1) : currentURL;
...
function variableHandling(name, type) {
    var modified = name;
...
window.onload = async function() {
    ...
    document.querySelector('#generate-api > button').addEventListener('click', function(){generateFullAPI("Flask")}, false);
}
